...
* To think about:
  - Is having split containers for building and running worth it?
    Might shrink size, but slightly annoying that one make command depends on other machine running.
    Could rewrite make rules, though, to fix this.
