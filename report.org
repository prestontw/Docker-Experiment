...
* Running webserver
  ...
  can pass -p <host port>:<container port> instead of -P for specific
  At least have to pass container port if using -p.

  Can expose port in dockerfile.
  Then -P will expose. Can see with sudo docker port <container name>
* To think about:
